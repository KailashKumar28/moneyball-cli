...
pub fn compute(
    ...
    let workspace = cfg
        .workspace
        .as_ref()
        .expect("brief called without workspace");
...
/// epoch bounds so compute_product doesn't have to take 4 epoch args.
struct Window {
    d0: NaiveDate,
    d1: NaiveDate,
    d0_ist: i64,
    d1_ist: i64,
    #[allow(dead_code)] // computed for parity with mb.py; not yet surfaced
    lag_cut_ist: i64,
}

fn compute_product(product: &str, snap: &Snapshot, window: &Window, goal: f64) -> ProductRow {
    let d0 = window.d0;
    let d1 = window.d1;
    let d0_ist = window.d0_ist;
    let d1_ist = window.d1_ist;
    ...
    let rs_per_q = if total_q > 0 {
        Some((total_spend / total_q as f64).round() as u64)
    } else {
        None
    };
    let l_to_q = if total_l > 0 {
        Some((total_q as f64 / total_l as f64) * 100.0)
    } else {
        None
    };
...
fn compute_feasibility(rows: &[ProductRow], cfg: &AppConfig, best_rpq: Option<u64>) -> Feasibility {
    ...
    let cur_rpq = if tot_q > 0.0 {
        (tot_spend_per_day as f64 / tot_q_per_day).round() as u64
    } else {
        0
    };
    let required_at_cur = if tot_q_per_day > 0.0 {
        Some(((tot_goal_per_day * cur_rpq as f64).round()) as u64)
    } else {
        None
    };
...
pub(crate) fn count_m_leads(actions: &[serde_json::Value]) -> u64 {
    ...
    let map: std::collections::HashMap<&str, &str> = actions
        .iter()
        .filter_map(|a| {
